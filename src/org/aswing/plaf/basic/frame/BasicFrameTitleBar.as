...
/**
 * The basic Imp of FrameTitleBar
 */
public class BasicFrameTitleBar extends Container implements FrameTitleBar, UIResource{
	...
	public function BasicFrameTitleBar(){
		super();
		titleEnabled = true;
		setLayout(new FrameTitleBarLayout());
		
		buttonPane = new Container();
		buttonPane.setLayout(new SoftBoxLayout(SoftBoxLayout.X_AXIS, 0));
	...
	protected function intallPropertiesFromOwnerUI():void{
	...
	private function __framePainted(e:AWEvent):void{
		if(titleLabel){
			titleLabel.setFont(owner.getFont());
			titleLabel.setForeground(owner.getForeground());
			titleLabel.paintImmediately();
		}
	}
